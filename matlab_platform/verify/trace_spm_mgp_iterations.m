...
while flag_MGP == 0 && traj_no < 1000
    ...
    if flag_MGP == 0
        ...
        item.update_theta = theta_update(:)';
        item.update_voltage = voltage_update(:)';
        if flag_update == 1
...
if ~isempty(trace)
    ...
    report.mgp_theta = trace(end).update_theta;
    report.mgp_voltage = trace(end).update_voltage;
end
report.trace = trace;
